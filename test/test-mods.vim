...
function! s:suite.should_open_in_new_tab() abort
  ...
  call s:expect(getline(1)).to_equal(printf('%s %s', g:dbui_icons.collapsed.db, 'dadbod_ui_test'))
  call s:expect(getline(2)).to_equal(printf('%s %s', g:dbui_icons.collapsed.db, 'dadbod_ui_testing'))
endfunction
